...
/* ===== Android/WebView scroll fix (seguro/minimal) ===== */
...
def do_logout():
    clear_auth_cookies()
    try:
        st.experimental_set_query_params()  # remove ?auth=
    except Exception:
        pass
...
def page_relatorio_dizimistas(user: "User"):
    ...
    with SessionLocal() as db:
        ...
        if not df_pesq.empty:
            ...
            st.metric("Dizimistas encontrados", f"{len(df_pesq)}")
            st.metric("Total geral da pesquisa", format_currency(tot_val))
...
def build_full_statement_pdf(cong_id: int, cong_name: str, ref: date) -> bytes:
    ...
    tithe_table_style = RLTableStyle([
        ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
        ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
        ("ALIGN", (0, 0), (-1, -1), "LEFT"),
...
def build_consolidated_pdf(agg_total: list, ref: date) -> bytes:
    ...
    tbl = RLTable(table_data, colWidths=[5*cm, 4*cm, 4*cm, 4*cm]); tbl.setStyle(table_style_main)
    story.append(tbl)
...
def build_missions_report_pdf(ref: date, entradas: list, saidas: list) -> bytes:
    ...
    if entradas:
        ...
        tbl_in = RLTable(entradas_data, colWidths=[9*cm, 9*cm]); tbl_in.setStyle(table_style)
        story.append(tbl_in)
    else:
    ...
    if saidas:
        ...
        tbl_out = RLTable(saidas_data, colWidths=[3*cm, 10*cm, 5*cm]); tbl_out.setStyle(table_style)
        story.append(tbl_out)
    else:
        ...
        with st.form("form_entrada_missoes", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar ENTRADA de Missões", type="primary"):
                with SessionLocal() as _db:
                    ...
                    _db.add(Transaction(
                        date=ent_data, type="DOAÇÃO", category_id=cat_obj.id,
