# main.py — AD Relatório Financeiro — Persistência de login + Mobile fix + Segment Control
...
/* Streamlit Cloud floating button safety space (no bloqueio de toque) */
@media (max-width: 900px){
  [data-testid="stDataFrame"] thead tr th{ position:static !important; top:auto !important; }
...
# Guardamos apenas o UID na URL; no load, se uid existir e for válido, restauramos a sessão.
...
# ===================== PAGES (código original mantido) =====================
# --- A partir daqui, as páginas são idênticas às tuas, com pequenos ajustes pontuais quando indicado ---
...
def page_lancamentos(user: "User"):
    ...
    with SessionLocal() as db:
        with st.sidebar:
            ...
            if st.button("Sair", on_click=do_logout):
                pass
...
# ---------------- CORE COLETA (mesmo do teu código, sem mudanças) ----------------
...
# ---------------- PÁGINAS RELATÓRIOS (iguais às tuas, somente com pequenas correções visuais) ----
# ... (Relatório de Entrada / Saída / Dizimistas / Missões e PDF builders)
# Para manter a resposta num tamanho razoável, mantive estes blocos exatamente como no seu código original.
# >>> COPIE os seus métodos 'page_relatorio_entrada', 'page_relatorio_saida', 'page_relatorio_dizimistas',
# >>> 'page_relatorio_missoes', 'build_full_statement_pdf', 'build_consolidated_pdf', 'build_missions_report_pdf'
# >>> sem alterações estruturais (apenas se já aplicamos algum fix antes, mantenha).

# ---------------- VISÃO GERAL (ajuste do TOP 5 sem duplicar) ----------------
...
def page_visao_geral(user: "User"):
    ...
    with SessionLocal() as db:
        ...
        # (PDF consolidado e prestação de contas — mantenha como no seu código original, se aplicável)

# ---------------- CADASTRO (igual ao teu, omitido aqui por tamanho) ----------------
# >>> COPIE o seu método page_cadastro sem alterações (apenas garantindo que o botão "Sair" usa on_click=do_logout)

# ===================== MAIN (roteamento com menu mobile) =====================
def main():
    try:
        ensure_seed()
        ...
        # Estado da página selecionada
        st.session_state.setdefault("page_sel", menu_options[0])

        # Sidebar (desktop)
        with st.sidebar:
            st.radio("Menu", options=menu_options, index=menu_options.index(st.session_state.page_sel),
                     key="menu_radio")
            # manter sincronizado
            st.session_state.page_sel = st.session_state.get("menu_radio", st.session_state.page_sel)

        # Mobile segmented control (parâmetro correto: default)
        st.markdown("<div class='mobile-nav'>", unsafe_allow_html=True)
        sel_m = st.segmented_control("Navegação", options=menu_options,
                                     default=st.session_state.page_sel, key="mobile_menu_nav")
        ...
        # espaço de segurança para botão flutuante do Streamlit Cloud (evita "travada" no fundo)
