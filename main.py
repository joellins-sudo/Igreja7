# main.py — Igreja Finance CHMS — v8.62
# Correções: sessão persiste após refresh (cookies com handshake de 1 rerun)
# - Inclui extra_streamlit_components.CookieManager
# - set_auth_cookies / clear_auth_cookies / bootstrap_auth_from_cookie
# - Botão "Sair" limpa cookies + session_state
# - Mantém todas as páginas do seu app
...
def set_auth_cookies(user: "User", days_valid: int = 30):
    ...
    cookie_manager.set(COOKIE_UID, str(user.id), expires_at=expires_at, key="set_uid")
    cookie_manager.set(COOKIE_VH, _verifier_from_password_hash(user.password_hash), expires_at=expires_at, key="set_vh")

def clear_auth_cookies():
    cookie_manager.delete(COOKIE_UID, key="del_uid")
    cookie_manager.delete(COOKIE_VH, key="del_vh")

def bootstrap_auth_from_cookie():
    """Handshake de cookies: 1º run monta o componente; 2º run os cookies chegam."""
    ...
    if st.button("Entrar", type="primary"):
        with SessionLocal() as db:
            ...
            if user and verify_password(p, user.password_hash):
                ...
                # >>> NOVO: grava cookies (30 dias)
                set_auth_cookies(user, days_valid=30)
                st.rerun()
            else:
...
def page_relatorio_entrada(user: "User"):
    ...
    with SessionLocal() as db:
        with st.sidebar:
            ...
            st.write(f"👤 **{user.username}** — *{user.role}*")
            if st.button("Sair"):
                clear_auth_cookies(); st.session_state.clear(); st.rerun()
...
def main():
    try:
        ...
        # >>> MUITO IMPORTANTE: recuperar autenticação dos cookies ANTES da UI
