# main.py — AD Relatório Financeiro — v9.2 (persistência de login com st.query_params)
...
# ---------- NOVO: Persistência via st.query_params ----------
def restore_user_from_url():
    """Se ?uid= estiver na URL e a sessão não tiver uid, restaura."""
    try:
    ...
    with SessionLocal() as db:
        ...
        if st.button("Criar usuário", key="cad_user_add"):
            with SessionLocal() as db2:
                if not u_user.strip() or not u_pwd.strip():
                ...
                elif db2.scalar(select(User).where(User.username == u_user.strip())):
                    ...
                    # Para o novo perfil, a congregação é sempre a Sede por padrão
                         ...
                         cong_id = db2.scalar(select(Congregation.id).where(Congregation.name == "Sede"))
                    
                    db2.add(User(username=u_user.strip(), password_hash=hash_password(u_pwd.strip()), role=u_role, congregation_id=cong_id))
                    db2.commit()
...
def main():
    try:
        ...
        # >>> Persistência: restaura uid da URL antes de checar login
        ...
        # Se está logado e a URL perdeu o uid, regrava
