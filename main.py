# main.py — Igreja Finance CHMS — v9.4 (completo)
# - Persistência de login (cookie + query param) + botão Sair
# - Removida mensagem “autenticado e persistente…”
# - Scroll fix p/ Android/WebView
# - PDF com RLTable/RLTableStyle
# - Sem experimental_* (usa st.query_params com fallback)
...
# ===== cookies (com fallback se o pacote não estiver disponível) =====
try:
...
# ===================== AUTH =====================
cookie_manager = stx.CookieManager() if _HAS_STX else None
...
def build_dizimista_search_pdf(df: pd.DataFrame, ano_pesq: int, cong_sel: str, mes_sel: str, nome_q: str) -> bytes:
    ...
    tbl = RLTable(data_table, colWidths=[3.5*cm, 3.5*cm, 2.0*cm, 2.5*cm, 2.5*cm, 2.0*cm, 2.0*cm])
...
def build_full_statement_pdf(cong_id: int, cong_name: str, ref: date) -> bytes:
    ...
    story.append(Paragraph("1. Entradas (Resumo: Dízimo e Oferta)", styles['Heading2']))
    if len(tx_in_data) > 1:
    ...
    story.append(Paragraph("2. Dizimistas (Lançamentos Nominais)", styles['Heading2']))
    if len(tithe_data) > 1:
        tbl_tithe = RLTable(tithe_data, colWidths=[3*cm, 9.5*cm, 4.5*cm])
        tbl_tithe.setStyle(RLTableStyle([
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTNAME", (0, 1), (-1, -1), "Helvetica"),
    ...
    story.append(Paragraph("3. Saídas", styles['Heading2']))
    if len(tx_out_data) > 1:
        tbl_out = RLTable(tx_out_data, colWidths=[2.5*cm, 4.5*cm, 7.5*cm, 3.5*cm])
        tbl_out.setStyle(RLTableStyle([
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTNAME", (0, 1), (-1, -1), "Helvetica"),
    ...
    summary_table = RLTable(summary_data, colWidths=[8*cm, 8*cm])
    ...
    story.append(Paragraph("4. Resumo Financeiro do Mês", styles['Heading2']))
...
def build_consolidated_pdf(agg_total: list, ref: date) -> bytes:
    ...
    tbl = RLTable(table_data, colWidths=[5*cm, 4*cm, 4*cm, 4*cm])
    tbl.setStyle(RLTableStyle([
        ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
        ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    story.append(Paragraph("Missões por Congregação (Entradas)", styles['Heading2']))
    ...
    tbl_m = RLTable(missions_table_data, colWidths=[9*cm, 9*cm])
    tbl_m.setStyle(RLTableStyle([
        ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
        ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
        ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
        ("ALIGN", (1, 1), (1, -1), "RIGHT"),
...
def build_missions_report_pdf(ref: date, entradas: list, saidas: list) -> bytes:
    ...
    story.append(Paragraph("Resumo de Entradas de Missões por Congregação", styles['Heading2']))
    if entradas:
        ...
        tbl_in = RLTable(entradas_data, colWidths=[9*cm, 9*cm])
        tbl_in.setStyle(RLTableStyle([
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    story.append(Spacer(1, 0.8*cm))
    story.append(Paragraph("Saídas de Missões", styles['Heading2']))
    if saidas:
        ...
        tbl_out = RLTable(saidas_data, colWidths=[3*cm, 10*cm, 5*cm])
        tbl_out.setStyle(RLTableStyle([
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    story.append(Paragraph("Resumo Financeiro de Missões", styles['Heading2']))
    summary_data = [
        ...
        with st.form("form_saida_missoes", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar SAÍDA", type="primary"):
                with SessionLocal() as _db:
                    ...
                    _db.add(Transaction(date=sai_data, type=TYPE_OUT, category_id=cat_obj.id,
                        amount=sai_valor, description=(sai_desc or None), congregation_id=sede_cong.id))
                    _db.commit()
...
def page_cadastro(user: "User"):
    ...
    with SessionLocal() as db:
        ...
        tithes_by_cong = dict(db.execute(select(Congregation.id, func.count(Tithe.id))
                                 ...
                                 .group_by(Congregation.id)).all())
        dfc = pd.DataFrame([{
            "ID": c.id, "Nome": c.name, "Usuários": int(users_by_cong.get(c.id, 0)),
            "Lançamentos": int(tx_by_cong.get(c.id, 0)), "Dízimos": int(tithes_by_cong.get(c.id, 0)),
        } for c in congs_all])
        if not dfc.empty:
