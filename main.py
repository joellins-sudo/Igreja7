        ...
        with st.form("form_saida", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar SAÍDA", type="primary"):
                with SessionLocal() as _db:
                    ...
                    _db.add(Transaction(
                        date=st.session_state["sai_data"],
                        type=TYPE_OUT,
                        category_id=cat_obj.id,
                        amount=st.session_state["sai_valor"],
                        description=(st.session_state["sai_desc"] or None),
                        congregation_id=cong_obj.id,
                    ))
                    _db.commit()
...
def build_consolidated_outputs_pdf(ref: date, saidas_por_categoria: dict) -> bytes:
    ...
    # Ordena as saídas para que "Missões (Saída)" apareça primeiro
    def sort_key(item):
        if _norm(item[0]) in ("missoes(saida)", "missões(saída)"):
            return (0, item[0])
        return (1, item[0])
    
    sorted_saidas = sorted(saidas_por_categoria.items(), key=sort_key)
...
def page_relatorio_saida(user: "User"):
    ...
    with SessionLocal() as db:
        ...
        if user.role == "SEDE":
            ...
            # st.info("Debug: Dados agregados para o PDF de Saídas")
            # st.write(saidas_por_categoria)
