# main.py — Igreja Finance CHMS — v8.53
# Ajustes: login persistente por query params, correção de rolagem mobile,
# remoção de tooltip duplicada nos cards do ranking e pequenas proteções.
...
html, body { background: var(--bg); height:auto!important; overflow-y:auto!important; -webkit-overflow-scrolling:touch; touch-action: pan-y; }
#root, [data-testid="stAppViewContainer"]{ height:auto!important; overflow:auto!important; -webkit-overflow-scrolling:touch; }
[data-testid="stAppViewContainer"] > .main{ overflow-y:auto!important; -webkit-overflow-scrolling:touch; }
div:focus{ outline:none!important; } /* evita travas de foco no mobile */
...
/* tooltip removido para evitar duplicação e possíveis travas de toque no mobile */
...
/* Segurança extra para mobile: garante que nenhum container oculto capture o toque */
...
# ===================== SESSION / LOGIN (persistência via query params) =====================
if "uid" not in st.session_state:
    st.session_state.uid = None

def _restore_uid_from_query_params():
    """Traz o uid da URL (?auth=<id>) quando a página é recarregada."""
    try:
        qp = st.query_params
        uid_qp = qp.get("auth")
        if uid_qp and not st.session_state.get("uid"):
            try:
                ...
                return
            with SessionLocal() as db:
                if db.get(User, uid_int):
                    st.session_state.uid = uid_int
    except Exception:
        pass

_restore_uid_from_query_params()
...
def do_logout():
    """Limpa sessão e parâmetros da URL e recarrega."""
    ...
    if st.button("Entrar", type="primary"):
        with SessionLocal() as db:
            ...
            if user and verify_password(p, user.password_hash):
                ...
                st.session_state.auth_ts = int(time.time())
                # persiste no URL
                try:
                    st.query_params.update({"auth": str(user.id), "t": str(st.session_state.auth_ts)})
                except Exception:
                    pass
                st.rerun()
            else:
...
def build_dizimista_search_pdf(df: pd.DataFrame, ano_pesq: int, cong_sel: str, mes_sel: str, nome_q: str) -> bytes:
    ...
    try:
        total_value = float(pd.to_numeric(df["Total no ano (R$)"], errors="coerce").fillna(0).sum())
    except Exception:
        total_value = 0.0
...
def page_relatorio_dizimistas(user: "User"):
    ...
    with SessionLocal() as db:
        ...
        if not df_pesq.empty:
            ...
            st.dataframe(df_pesq.assign(**{"Total no ano (R$)": df_pesq["Total no ano (R$)"].map(lambda x: float(x))}).assign(**{"Total no ano (R$)": lambda d: d["Total no ano (R$)"].map(format_currency)}), use_container_width=True, hide_index=True, height=300)
            tot_reg = len(df_pesq)
            try:
                tot_val = float(pd.to_numeric(df_pesq["Total no ano (R$)"], errors="coerce").fillna(0).sum())
            except Exception:
                tot_val = 0.0
            cA, cB = st.columns(2)
...
def build_consolidated_pdf(agg_total: list, ref: date) -> bytes:
    ...
    for c_name, entradas, saidas, saldo, missoes in agg_total:
        table_data.append([c_name, format_currency(entradas), format_currency(sd:=saidas), format_currency(saldo)])
        total_entradas += entradas; total_saidas += sd; total_saldo += saldo
